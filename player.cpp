...
Player::Player(Ball* ball0, Table* table) : GameObject(PLAYER, vec3f()), ball_(ball0), 
	...
	cue_visible_(true), camera_target_(), camera_pos_(20.f, PI/4.f, 0.f), 
	state_(3), state_count_(0), shot_power_(0.1f), shot_power_max_(1.5f), 
	display_guide_(true), turn_(0)
{
	...
	Sound::GetInstance().Set3DListener(camera_pos_.ToVec3f() + camera_target_, camera_target_);
}
...
void Player::Update()
{
	...
	case 1: // ショットの強さを決める
		...
		if (mouse_.LeftDown()) {
			...
			if (turn_ == 0) {
				// ブレイクショットの場合は速度2倍
				ball_->SetVel(shot_dir.Normalize() * shot_power_ * 2.f);
			} else {
			...
			shot_power_ = 0.1f;
			++turn_;
			Sound::GetInstance().Play3D("shot", ball_->GetPos());
			break;
		}
		// 右クリックでキャンセル
		if (mouse_.RightDown()) {
